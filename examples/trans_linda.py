...
ev_otu = "/media/veracrypt1/gLinDA/gLinDA/examples/EV_OTU.csv"
ev_meta = "/media/veracrypt1/gLinDA/gLinDA/examples/EV_META.csv"

ev_otu_df = pd.read_csv(ev_otu, index_col="ID")
ev_meta_df = pd.read_csv(ev_meta, index_col="ID")

print(ev_otu_df)
print(ev_meta_df)

def replace_zeros_with_halfmin(row):
    min_val = 0.5 * row[row != 0].min()
    row[row == 0] = min_val
    return row
...
def trans_linda(feature_dat, meta_dat, formula,
    ...
    feature_dat.index = feature_dat.index.map(str)
    ...
    #Y.to_csv("~/pre_zerohandling_Y_p.csv")
    # Y.sort_index(inplace = True, axis=1)
    ...
    if feature_dat_type == 'count':
        if (Y == 0).any().any():
            ...
            if adaptive:
                logN = np.log(N)
                # print((set(logN[0])- set(Z.index)) & (set(Z.index) - set(logN[0])))
                if random_effect:
            ...
            if zero_handling == "Imputation":
                ...
                Y = Y + pseudo_count
    #Y.to_csv("~/fLinDA/meantestp.csv")
    
    if feature_dat_type == "proportion":
        ind_name = Y.index.name
        if (Y == 0).any().any():

            Y.apply(replace_zeros_with_halfmin, axis=1)
            Y.columns = samp_name
            Y.index = taxa_name
            Y.index.name = ind_name

    ##############################################################################
    # CLR transform
    logY = np.log2(Y)
    W = logY - np.mean(logY, axis=0)
    W = W.T
    ...
    for voi in list(set(res.index.values)):
        ...
        reject, padj = mult.fdrcorrection(pval, alpha=alpha)
...
param_grid = {'prev_filter': [0.01, 0.1, 0.2], 'mean_abund_filter' : [0.01, 0.1, 0.2], 'max_abund_filter': [0, 0.1]}
grid = ParameterGrid(param_grid)

linda_out = trans_linda(ev_otu_df, ev_meta_df,
                        formula="~ GOLD + SMOKING",
                        model_output_path="/home/leon/fLinDA/test_models",
                        data_name="EV", prev_filter=0.1, mean_abund_filter=0.,
                        max_abund_filter=0, feature_dat_type="count")

out = linda_out["output"]["GOLD"]#.sort_values(by=["padj"])
grid_res_tbl = pd.DataFrame(index=out.index)
cnt = 0
...
for params in grid:
	try:
		linda_out = trans_linda(ev_otu_df, ev_meta_df,
		                formula="~ GOLD + SMOKING",
		                model_output_path="/home/leon/fLinDA/test_models",
		                data_name="EV", prev_filter=params["prev_filter"], mean_abund_filter=params["mean_abund_filter"],
		                max_abund_filter=params["max_abund_filter"], feature_dat_type="count")
		grid_res_tbl["r%d" % cnt] = out["padj"]
		cnt += 1
	except:
		continue
