...
# cdi_otu = "/home/leon/fLinDA/cdiotu.csv"
# cdi_meta = "/home/leon/fLinDA/cdimeta.csv"

# ibd_otu = "/home/leon/fLinDA/ibdotu.csv"
# ibd_meta = "/home/leon/fLinDA/ibdmeta.csv"

# ra_otu = "/home/leon/fLinDA/raotu.csv"
# ra_meta = "/home/leon/fLinDA/rameta.csv"

smoke_otu = "smokeotu.csv"
smoke_meta = "smokemeta.csv"

# cdi_otu_df = pd.read_csv(cdi_otu, index_col="ID")
# cdi_meta_df = pd.read_csv(cdi_meta, index_col="SampleID")

# ibd_otu_df = pd.read_csv(ibd_otu, index_col="ID")
# ibd_meta_df = pd.read_csv(ibd_meta, index_col="SampleID")

# ra_otu_df = pd.read_csv(ra_otu, index_col="ID")
# ra_meta_df = pd.read_csv(ra_meta, index_col="SampleID")

smoke_otu_df = pd.read_csv(smoke_otu, index_col="ID")
smoke_meta_df = pd.read_csv(smoke_meta, index_col="SampleID")
...
# print(winsor_fun(ibd_otu_df, quan=0.60, feature_dat_type='count'))


def trans_linda(feature_dat, meta_dat, formula,
                feature_dat_type = 'count', data_name = "", model_output_path="", prev_filter = 0,
    ...
    keep_tax = (((temp != 0).mean(axis=1) >= prev_filter) & (temp.mean(axis=1) >= mean_abund_filter) & (temp.max(axis=1) >= max_abund_filter))
    if verbose:
    ...
    # winsorization
    if is_winsor:
        Y = winsor_fun(Y, 1 - outlier_pct, feature_dat_type)
    ...
    for voi in list(set(res.index.values)):
        ...
        pre_bias, iters = default_mean_shift_modeest(math.sqrt(float(n)) * highdec_log2FoldChange)
    ...
    return {"variables": variables, "biases": biases, "output": output_frames, "feature_data_used": Y, "meta_data_used": Z}


result = trans_linda(smoke_otu_df, smoke_meta_df,
                  formula="~ Smoke + Sex + Site + numervar",
                  model_output_path="test_models",
                  data_name="Smoke", prev_filter=0.1, mean_abund_filter=0,
                  max_abund_filter=0, feature_dat_type="count")


print(sum(result["output"]["numervar"]["log2FoldChange"]))

# test_vec = [1.85640060, 3.81578556, 1.06500855, 0.70691183, 3.88247013, 0.25031027, 3.24851609, 2.92172981, 4.60241382, 0.00374607]

# test_vec = trans_linda(smoke_otu_df, smoke_meta_df, formula="~ Smoke + Sex + Site + numervar", model_output_path="/home/leon/fLinDA/test_models", data_name="Smoke", prev_filter=0.10, mean_abund_filter=0, max_abund_filter=0, feature_dat_type="count")
# pd.DataFrame(test_vec).to_csv("test_vec.csv")

# test_vec = pd.read_csv("~/fLinDA/test_vec.csv")
# test_vec = np.array(test_vec["log2val"].values)
# print(default_shorth(test_vec))
# print(default_mean_shift_modeest(test_vec * 12.04159)[1])
# print(default_mean_shift_modeest(test_vec * 12.04159)[0] / 12.04159)
